...
#use this macro for final analysis
import sys
sys.path.append('/work/clas12/bclary/CLAS12/analysis_code_fork0/projects/source/')
...
h_trento_bins = TH1F('h_trento_bins','h_trento_bins',10,-180.0,180.0)
...
ccounter =0
phi_fout = open('final_phi_events_for_unbinned.txt','w+')
...
for ev in ff.clas12:
    ...
    pr_detector_status = 'FD' if pr_status >= 2000 and pr_status < 4000  else 'CD'
    kp_detector_status = 'FD' if kp_status >= 2000 and kp_status<4000 else 'CD'
    km_detector_status = 'FD' if km_status >= 2000 and km_status<4000 else 'CD'
    ...
    test_excl_cut_results = (  excl_cuts.passEPKPKMX(epkpkmX),
                              excl_cuts.passEPKPX(epkpX),
                              excl_cuts.passEKPKMX(ekpkmX),
                              excl_cuts.passEPKMX(epkmX)
                          )
    if( all(test_excl_cut_results) ):
        cut_name='pass_all_testing'
        compute_if_absent(histos,'el_ptheta_'+pr_detector_status+'_'+kp_detector_status+'_'+km_detector_status+'_'+cut_name,builders['ptheta']).Fill( el.P(),  np.degrees(el.Theta()) )
        compute_if_absent(histos,'pr_ptheta_'+pr_detector_status+'_'+kp_detector_status+'_'+km_detector_status+'_'+cut_name,builders['ptheta']).Fill( pro.P(),  np.degrees(pro.Theta()) )
        compute_if_absent(histos,'kp_ptheta_'+pr_detector_status+'_'+kp_detector_status+'_'+km_detector_status+'_'+cut_name,builders['ptheta']).Fill( kp.P(),  np.degrees(kp.Theta()) )
        compute_if_absent(histos,'km_ptheta_'+pr_detector_status+'_'+kp_detector_status+'_'+km_detector_status+'_'+cut_name,builders['ptheta']).Fill( km.P(),  np.degrees(km.Theta()) )
    ...
    if( all_final_state_status_FD ):
        ...
        if( all(exclusivity_cut_results2) ):
            ...
            if( remove_lambda and remove_lambda2):
                ...
                ## not necessary to remove reconsances as the sideband subtraction technique will do so
            ...
            ## the following lines below are to check the phi meson inv. mass per trento bin, to calculate the number of events and background per bin in trento.
            trento_phi_bin = h_trento_bins.FindBin(trento)
            if( ev.hel > 0 ):
                compute_if_absent(histos,'vphimass_pos_helicity_trentobin_'+str(trento_phi_bin)+'_pass_all_exclusivity',builders['mass']).Fill(vphi.M())
            if( ev.hel < 0 ):
                compute_if_absent(histos,'vphimass_neg_helicity_trentobin_'+str(trento_phi_bin)+'_pass_all_exclusivity',builders['mass']).Fill(vphi.M())
            ...
            phi_fout.write( str(vphi.M()) + ' ' + str(ev.hel) + ' ' + str(trento) + ' \n')
...
fout = TFile('pass1_exclusive_phi_kinematics_final_ebc_heb_FDonly_'+tag+'_'+base,'recreate')
