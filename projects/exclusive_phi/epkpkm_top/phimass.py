...
# was [850,1.019, 0.015, 0.987, 0.945, 50, 2,2]
...
for ii in range(1, len(xaxis_range)-1):
    ...
    xx_from_peak =  xaxis_range[ii]
    int_bck = bck.Integral(xx_min, xx_max)
    int_gaus = sig_gaus.Integral(xx_min, xx_max)
...
xx_side_bands = []
xx_sigma_sb = np.arange(1, 2.5, 0.5)
print(xx_sigma_sb)
sb=0
delta_kpkm_sb = 0.0482
for ss in xx_sigma_sb:
    xx_side_bands.append( [mean - sigma*ss, mean + sigma*ss] )

    int_bck = bck.Integral(mean - sigma*ss, mean + sigma*ss)
...
min_range_to_use = 1.0126
max_range_to_use = 1.0268
int_cal_bck = bck.Integral(min_range_to_use, max_range_to_use)
