...
phi_mass_resolution = 5.647e-03# 4.25752e-03  
...
bin_range = int(math.floor(2*phi_mass_resolution*sigma_range/0.007))
print(' bin range %f ' % (bin_range))
...
for ev in ff.clas12:
    ...
    if( all_final_state_status_FD ):
        ...
        if( all(exclusivity_cut_results2) ):
            ...
            phi_mass_min_limit = max_phi_mass_signal_limit
            min_bin_mass = max_bin_min_phi
            for ii in range(1,26):

                max_bin_mass = min_bin_mass + bin_range
                max_phi_mass=0

                phi_mass_max_limit = phi_mass_min_limit + 2*phi_mass_resolution*sigma_range
                
                #print(' min %f , max %f ' % (phi_mass_min_limit, phi_mass_max_limit))


                #if( bin_phimass >= min_bin_mass and bin_phimass < max_bin_mass ):
                if( remove_lambda and remove_lambda2 and vphi.M() > phi_mass_min_limit and vphi.M() < phi_mass_max_limit ):
                ...
                phi_mass_min_limit=phi_mass_max_limit
