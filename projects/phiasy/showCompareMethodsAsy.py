...
def plot_asy_methods( can, file_in_method1, file_in_method2, save_name, label, title, xtitle, ytitle, bin_info_method1=None, bin_info_method2=None ):
    ...
    leg.AddEntry(asy_graph_m1,'Method1','p')
    leg.AddEntry(asy_graph_m2,'Method2','p')
    leg.Draw('same')
    dumpster.append(leg)

    ## draw asy fit information between two methods
    label.DrawLatex(0.13, 0.85, 'Asy_{m1}'+'{0:.4f} #pm {1:.4f}'.format(fit_sin_m1.GetParameter(0), fit_sin_m1.GetParError(0)))
    label.DrawLatex(0.13, 0.79, '#chi^{2}_{m1} /NDF'+'{0:2.1f} / {1}'.format(fit_sin_m1.GetChisquare(), fit_sin_m1.GetNDF()))

    label.DrawLatex(0.13, 0.73, 'Asy_{m2}'+'{0:.4f} #pm {1:.4f}'.format(fit_sin_m2.GetParameter(0), fit_sin_m2.GetParError(0)))
    label.DrawLatex(0.13, 0.67, '#chi^{2}_{m2} /NDF'+'{0:2.1f} / {1}'.format(fit_sin_m2.GetChisquare(), fit_sin_m2.GetNDF()))
...
if __name__ == '__main__':
    ...
    fin_m1 = open('asy_vs_trento_method1_imkpkm_bin2_pidtype1_additionalcuts_binVar6_'+field_setting+'.txt','r')
    fin_m2 = open('asy_method2_integrated_additionalcuts_binVar6_'+field_setting+'.txt','r')
    plot_asy_methods( can, fin_m1, fin_m2, output_pdfname, lab, 'BSA Method Comparison', '#phi_{trento} (deg)', 'Asy')
