...
class epkpkm {
	...
	if( this.data_type == 1 ){
	    t1 = f1.makeNtuple('clas12','clas12phi',phyev.genTreeString()+":"+phyev.genMCTreeString())	    
	}
	else{
    ...
    def fillMCOutTree(int event_rec, LorentzVector el, LorentzVector pr, LorentzVector kp, LorentzVector km, int hel, LorentzVector mcel, LorentzVector mcpr, LorentzVector mckp, LorentzVector mckm, int mchel){
 	t1.fill(event_rec,
		el.px(),
		el.py(),
		el.pz(),
...
//	println('here ')
	...
	if( ep.size() == 1 ){	    
		...
		(0..<event.npart).findAll{event.pid[it]==321}.findResults{ikp->
 		    def kp = LorentzVector.withPID(321,event.px[ikp], event.py[ikp], event.pz[ikp]) 		
		    def mkp = [kp_index:ikp, lv:kp]
		    return [mele,mpro,mkp]
		}
            }.collectMany{mele,mpro,mkp->  
		(0..<event.npart).findAll{event.pid[it]==-321}.findResults{ikm->
		    def km = LorentzVector.withPID(-321,event.px[ikm], event.py[ikm], event.pz[ikm]) 						
		    def mkm = [km_index:ikm, lv:km]
	...
	if(data_type == 1){
	    fillMCOutTree(event_rec,final_el,final_pr,final_kp,final_km,-1000,final_mcel,final_mcpr,final_mckp,final_mckm,-1000)
	}
