...
from ROOT import (TH1F, TH2F, TF1, TFile, TCanvas,
                  ...
                  TLine, kRed, kBlack, kBlue, kWhite)
...
if __name__ == '__main__':
    ...
    input_dataEB_rootfile = args.input_dataEB_file
    input_dataMod_rootfile = args.input_dataMod_file

    output_pdfname = args.output_prefix + '.pdf'
    rootfile_dataEB = TFile(input_dataEB_rootfile)
    rootfile_dataMod = TFile(input_dataMod_rootfile)
    histos_eb = load_histos(rootfile_dataEB)
    histos_mod = load_histos(rootfile_dataMod)
    ...
    '''
    ## first compare the final state selected EB and modPID events in data
    # electrons
    ...
    # compare:
    # 1 - electron eb and hadron eb
    # 2 - electron eb and hadron eb + fid
    # 3 - electron mod and hadron eb
    # 4 - electron mod and hadron eb + fid
    compare_type = 'El_{mod}, No Fid vs Fid'

    #electron
    plot_overlap_1d( can, histos_eb, histos_mod, 'p_ele_pass_all_cut_phi_mass', output_pdfname, lab, 
                     title=compare_type+', P_{e} Final #phi (FD)' , xtitle='P_{el} (GeV)', ytitle='counts', logy=True, set_marker_style=True)

    plot_overlap_1d( can, histos_eb, histos_mod, 'theta_ele_pass_all_cut_phi_mass', output_pdfname, lab, 
                     title=compare_type+', #Theta_{e} Final #phi (FD)' , xtitle='#Theta_{el} (deg)', ytitle='counts', logy=True, set_marker_style=True)

    plot_overlap_1d( can, histos_eb, histos_mod, 'phi_ele_pass_all_cut_phi_mass', output_pdfname, lab, 
                     title=compare_type+', #Phi_{e} Final #phi (FD)' , xtitle='#Phi_{el} (deg)', ytitle='counts', logy=True, set_marker_style=True)
    #proton
    plot_overlap_1d( can, histos_eb, histos_mod, 'p_pro_pass_all_cut_phi_mass', output_pdfname, lab, 
                     title=compare_type+', P_{p} Final #phi (FD)' , xtitle='P_{p} (GeV)', ytitle='counts', logy=True, set_marker_style=True)

    plot_overlap_1d( can, histos_eb, histos_mod, 'theta_pro_pass_all_cut_phi_mass', output_pdfname, lab, 
                     title=compare_type+', #Theta_{p} Final #phi (FD)' , xtitle='#Theta_{p} (deg)', ytitle='counts', logy=True, set_marker_style=True)

    plot_overlap_1d( can, histos_eb, histos_mod, 'phi_pro_pass_all_cut_phi_mass', output_pdfname, lab, 
                     title=compare_type+', #Phi_{p} Final #phi (FD)' , xtitle='#Phi_{p} (deg)', ytitle='counts', logy=True, set_marker_style=True)
    #kaonsP
    plot_overlap_1d( can, histos_eb, histos_mod, 'p_kp_pass_all_cut_phi_mass', output_pdfname, lab, 
                     title=compare_type+', P_{K^{+}} Final #phi (FD)' , xtitle='P_{K^{+}} (GeV)', ytitle='counts', logy=True, set_marker_style=True)

    plot_overlap_1d( can, histos_eb, histos_mod, 'theta_kp_pass_all_cut_phi_mass', output_pdfname, lab, 
                     title=compare_type+', #Theta_{K^{+}} Final #phi (FD)' , xtitle='#Theta_{K^{+}} (deg)', ytitle='counts', logy=True, set_marker_style=True)

    plot_overlap_1d( can, histos_eb, histos_mod, 'phi_kp_pass_all_cut_phi_mass', output_pdfname, lab, 
                     title=compare_type+', #Phi_{K^{+}} Final #phi (FD)' , xtitle='#Phi_{K^{+}} (deg)', ytitle='counts', logy=True, set_marker_style=True)

    #kaonM
    plot_overlap_1d( can, histos_eb, histos_mod, 'p_km_pass_all_cut_phi_mass', output_pdfname, lab, 
                     title=compare_type+', P_{K^{-}} Final #phi (FD)' , xtitle='P_{K^{-}} (GeV)', ytitle='counts', logy=True, set_marker_style=True)

    plot_overlap_1d( can, histos_eb, histos_mod, 'theta_km_pass_all_cut_phi_mass', output_pdfname, lab, 
                     title=compare_type+', #Theta_{K^{-}} Final #phi (FD)' , xtitle='#Theta_{K^{-}} (deg)', ytitle='counts', logy=True, set_marker_style=True)

    plot_overlap_1d( can, histos_eb, histos_mod, 'phi_km_pass_all_cut_phi_mass', output_pdfname, lab, 
                     title=compare_type+', #Phi_{K^{-}} Final #phi (FD)' , xtitle='#Phi_{K^{-}} (deg)', ytitle='counts', logy=True, set_marker_style=True)

    ##### compare final invariant mass dist of kp to km. This is important to see what is being lost or gained
    plot_overlap_1d( can, histos_eb, histos_mod, 'im_kpkm_pass_all', output_pdfname, lab, 
                     title=compare_type+', I.M. K^{+}K^{-} Final #phi (FD)' , xtitle='I.M. K^{+}K^{-} (GeV)', ytitle='counts', logy=True, set_marker_style=True)
    
    ##### now look at q2, xb, -t
    plot_overlap_1d( can, histos_eb, histos_mod, 'q2_pass_all_cut_phi_mass', output_pdfname, lab, 
                     title=compare_type+', Q^{2} Final #phi (FD)' , xtitle='#Q^{2} (GeV^{2})', ytitle='counts', logy=True, set_marker_style=True)

    plot_overlap_1d( can, histos_eb, histos_mod, 't_pass_all_cut_phi_mass', output_pdfname, lab, 
                     title=compare_type+', -t Final #phi (FD)' , xtitle='-t (GeV^{2})', ytitle='counts', logy=True, set_marker_style=True)
    
    plot_overlap_1d( can, histos_eb, histos_mod, 'xb_pass_all_cut_phi_mass', output_pdfname, lab, 
                     title=compare_type+', X_{b} Final #phi (FD)' , xtitle='X_{b}', ytitle='counts', logy=True, set_marker_style=True)

    #plot_overlap_1d( can, histos_eb, histos_mod, 'phitrento_small_pass_all_cut_phi_mass', output_pdfname, lab, 
    #                 title=compare_type+', #phi_{Trento} Final #phi (FD)' , xtitle='#phi_{trento} (deg)', ytitle='counts', logy=True, set_marker_style=True)

    plot_overlap_1d( can, histos_eb, histos_mod, 'phitrento_pass_all_cut_phi_mass', output_pdfname, lab, 
                     title=compare_type+', #phi_{Trento} Final #phi (FD)' , xtitle='#phi_{trento} (deg)', ytitle='counts', logy=True, set_marker_style=True)
