...
class EventConverter {
	...
	if (dataEvent.hasBank("RUN::scaler")){
	    def scal = dataEvent.getBank("RUN::scaler")
	    event.fcup_gated=scal.getFloat("fcupgated",0)
	    event.fcup_ungated=scal.getFloat("fcup",0)
	    event.live_time=scal.getFloat("livetime",0)
	}
        ...
        if (dataEvent.hasBank("REC::Calorimeter")){
            ...
            (0 ..< cal.rows()).each{ index ->
                ...
                if (layer == 1){
                    ...
                    event.pcal_m2u.put(pindex, cal.getFloat('m2u', index))
                    event.pcal_m2v.put(pindex, cal.getFloat('m2v', index))
                    event.pcal_m2w.put(pindex, cal.getFloat('m2w', index))
		    event.pcal_x.put(pindex, cal.getFloat('x',index))
		    event.pcal_y.put(pindex, cal.getFloat('y',index))
		    event.pcal_z.put(pindex, cal.getFloat('z',index))
                }
                else if (layer == 4){
                    ...
                    event.ecal_inner_m2u.put(pindex, cal.getFloat('m2u', index))
                    event.ecal_inner_m2v.put(pindex, cal.getFloat('m2v', index))
                    event.ecal_inner_m2w.put(pindex, cal.getFloat('m2w', index))
		    event.ecal_inner_x.put(pindex, cal.getFloat('x',index))
		    event.ecal_inner_y.put(pindex, cal.getFloat('y',index))
		    event.ecal_inner_z.put(pindex, cal.getFloat('z',index))
                }
                else if (layer == 7){
                    ...
                    event.ecal_outer_m2u.put(pindex, cal.getFloat('m2u', index))
                    event.ecal_outer_m2v.put(pindex, cal.getFloat('m2v', index))
                    event.ecal_outer_m2w.put(pindex, cal.getFloat('m2w', index))
		    event.ecal_outer_x.put(pindex, cal.getFloat('x',index))
		    event.ecal_outer_y.put(pindex, cal.getFloat('y',index))
		    event.ecal_outer_z.put(pindex, cal.getFloat('z',index))
                }
            }
        }
    }
        ...
        if (dataEvent.hasBank("REC::Traj")){
            ...
            (0 ..< traj.rows()).collect{ index ->
                ...
                hits.each{ hit ->
                    ...
                    else if (hit.layer == 18 ){// >= 13 && hit.layer < 25){
