...
electron = new ElectronFromEvent();
...
pass_dc_comb=[true,true,true,true,false,false,false,true,true,true]
// charge, status and pass PCAL fid DCr1, r2, r3 fid cuts
pass_dc_pcal_comb=[true,true,true,true,false,true,false,true,true,true]
pass_cut_comb = [0:pass_dc_comb,
		 1:pass_dc_pcal_comb]
...
for(fname in args) {
    ...
    while(reader.hasEvent() && cc < 2000000) {
	...
	my_el_cuts.each{ index, value ->
		...
		value.eachWithIndex{ cut_result, cut_index ->
		    ...
		    histos.computeIfAbsent("chi_cut$cut_index"+"_s$dc_sector",histoBuilders.tchi2).fill(dc_chi2)
 		    ...
 		    //histos.computeIfAbsent("delta_energy_cut$cut_index"+"_$dc_sector",histoBuilders.deltakin).fill(delta_energy)
		    //histos.computeIfAbsent("delta_theta_cut$cut_index"+"_$dc_sector",histoBuilders.deltakin).fill(delta_theta)
 		    //histos.computeIfAbsent("delta_energy_vs_chi2_cut$cut_index"+"_$dc_sector",histoBuilders.deltakin2D).fill(dc_chi2,delta_energy)
		    //histos.computeIfAbsent("delta_theta_vs_chi2_cut$cut_index"+"_$dc_sector",histoBuilders.deltakin2D).fill(dc_chi2,delta_theta)
		    //histos.computeIfAbsent("delta_energy_vs_p_cut$cut_index"+"_$dc_sector",histoBuilders.deltakin2D_beam).fill(p,delta_energy)
		    //histos.computeIfAbsent("delta_theta_vs_p_cut$cut_index"+"_$dc_sector",histoBuilders.deltakin2D_beam).fill(p,delta_theta)
		}	    	
		if(!value.contains(false)){
	...
	if(event.npart>0){		
	    ...
	    event.charge.findResults{ it.value < 0 ? it.value : null}?.each{histos.computeIfAbsent("event_charge",histoBuilders.helicity).fill(it)}
	    event.charge.findResults{ it.value > 0 ? it.value : null}?.each{histos.computeIfAbsent("event_charge",histoBuilders.helicity).fill(it)}
	}		
    }
    ...
    out.writeFile("electron_pid_test.hipo")
