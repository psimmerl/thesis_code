...
//el_p_passall_pidchi2
	...
	for( part in identified_particles ){
		...
		def p_range = -1
		/*
		for( int p_bin = 1; p_bin < 120; p_bin++ ){
		    p_max = delta_p*p_bin
		    if( p < p_max && p > p_min ){
			p_range = p_bin
		    }
		...
		value.eachWithIndex{ cut_result, cut_index ->		
			...
			if( eocal_sect >= 0 ){
 			...
 			if(cal_sector >= 0 ){
			    ...
			    /*
			    if( p_range >= 0 ){
				histos.computeIfAbsent(p_name+"_"+"calsf_pcalvsecal_prange"+"$p_range"+"_cut$cut_index",histoBuilders.calsf).fill(pcaldep/p, (eidep + eodep)/p)			    
				//histos.computeIfAbsent(p_name+"_"+"calsf_modsquare_pcalvsecal_prange"+"$p_range"+"_cut$cut_index",histoBuilders.calsf).fill(Math.pow(pcaldep/p,2), Math.pow((eidep + eodep)/p,2))			    
				//histos.computeIfAbsent(p_name+"_"+"calsf_modsquare_pcalvsei_prange"+"$p_range"+"_cut$cut_index",histoBuilders.calsf).fill(Math.pow(pcaldep/p,2), Math.pow(eidep/p,2))

				histos.computeIfAbsent(p_name+"_"+"calsfchi_prange"+"$p_range"+"_cut$cut_index",histoBuilders.ecsf_chi).fill(pidchi2) 	
				histos.computeIfAbsent(p_name+"_"+"calsfchi_prange"+"$p_range"+"_cut$cut_index"+"_s$cal_sector",histoBuilders.ecsf_chi).fill(pidchi2) 	
				
			     if( eodep > 0 && eidep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsf_eivseo_prange"+"$p_range"+"_cut$cut_index",histoBuilders.calsf).fill(eidep/p, eodep/p)
				if( pcaldep > 0 && eidep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsf_pcalvsei_prange"+"$p_range"+"_cut$cut_index",histoBuilders.calsf).fill(pcaldep/p, eidep/p)
				if( pcaldep > 0 && eidep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsf_eivspcal_prange"+"$p_range"+"_cut$cut_index",histoBuilders.calsf).fill(eidep/p, pcaldep/p)
				if( pcaldep > 0 && eodep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsf_pcalvseo_prange"+"$p_range"+"_cut$cut_index",histoBuilders.calsf).fill(pcaldep/p, eodep/p)
				histos.computeIfAbsent(p_name+"_"+"calsf_eivseo_prange"+"$p_range"+"_cut$cut_index",histoBuilders.calsf).fill(eidep/p, eodep/p)
				histos.computeIfAbsent(p_name+"_"+"calsf_pcalvsei_prange"+"$p_range"+"_cut$cut_index",histoBuilders.calsf).fill(pcaldep/p, eidep/p)
				histos.computeIfAbsent(p_name+"_"+"calsf_eivspcal_prange"+"$p_range"+"_cut$cut_index",histoBuilders.calsf).fill(eidep/p, pcaldep/p)
				histos.computeIfAbsent(p_name+"_"+"calsf_pcalvseo_prange"+"$p_range"+"_cut$cut_index",histoBuilders.calsf).fill(pcaldep/p, eodep/p)
			    }
			     */
				...
				histos.computeIfAbsent(p_name+"_"+"calsfchi_bigp_cut$cut_index"+"_s$cal_sector",histoBuilders.ecsf_chi).fill(pidchi2) 	
 				...
 				/*if( eodep > 0 && eidep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsf_eivseo_bigp_cut$cut_index",histoBuilders.calsf).fill(eidep/p, eodep/p)
				if( pcaldep > 0 && eidep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsf_pcalvsei_bigp_cut$cut_index",histoBuilders.calsf).fill(pcaldep/p, eidep/p)
				if( pcaldep > 0 && eodep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsf_pcalvseo_bigp_cut$cut_index",histoBuilders.calsf).fill(pcaldep/p, eodep/p)*/
		...
		if( eocal_sect >= 0 ){
 		...
 		if(true){//cal_sector >= 0 ){
			...
			/*if( eodep > 0 && eidep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsf_eivseo_passall",histoBuilders.calsf).fill(eidep/p, eodep/p)
			if( pcaldep > 0 && eidep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsf_pcalvsei_passall",histoBuilders.calsf).fill(pcaldep/p, eidep/p)
			if( pcaldep > 0 && eodep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsf_pcalvseo_passall",histoBuilders.calsf).fill(pcaldep/p, eodep/p)
			if( pcaldep > 0 && eidep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsfmod_pcalvsei_passall",histoBuilders.calsf).fill(pcaldep/p, eidep)*/
			...
			for( int sig_region = 1; sig_region <= 6; sig_region++ ){			
				...
				/*
				if( eodep > 0 && eidep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsf_eivseo_passall"+"pidchi2_$sig_region"+"sig",histoBuilders.calsf).fill(eidep/p, eodep/p)
				if( pcaldep > 0 && eidep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsf_pcalvsei_passall"+"pidchi2_$sig_region"+"sig",histoBuilders.calsf).fill(pcaldep/p, eidep/p)
				if( pcaldep > 0 && eidep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsfmod_pcalvsei_passall"+"pidchi2_$sig_region"+"sig",histoBuilders.eieo).fill(pcaldep/p, eidep)
				if( pcaldep > 0 && eidep > 0 ) histos.computeIfAbsent(p_name+"_"+"mod_pcalvsei_passall"+"pidchi2_$sig_region"+"sig",histoBuilders.eieo).fill(pcaldep, eidep)
				if( pcaldep > 0 && eodep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsf_pcalvseo_passall"+"pidchi2_$sig_region"+"sig",histoBuilders.calsf).fill(pcaldep/p, eodep/p)
				if( pcaldep > 0 && eodep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsfmod_pcalvseo_passall"+"pidchi2_$sig_region"+"sig",histoBuilders.eieo).fill(pcaldep/p, eodep)
				if( pcaldep > 0 && eodep > 0 ) histos.computeIfAbsent(p_name+"_"+"mod_pcalvseo_passall"+"pidchi2_$sig_region"+"sig",histoBuilders.eieo).fill(pcaldep, eodep)
				 */
				...
				/*
				if( eodep > 0 && eidep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsf_eivseo_passall"+"failpidchi2_$sig_region"+"sig",histoBuilders.calsf).fill(eidep/p, eodep/p)
				if( pcaldep > 0 && eidep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsf_pcalvsei_passall"+"failpidchi2_$sig_region"+"sig",histoBuilders.calsf).fill(pcaldep/p, eidep/p)
				if( pcaldep > 0 && eidep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsfmod_pcalvsei_passall"+"failpidchi2_$sig_region"+"sig",histoBuilders.eieo).fill(pcaldep/p, eidep)
				 if( pcaldep > 0 && eidep > 0 ) histos.computeIfAbsent(p_name+"_"+"mod_pcalvsei_passall"+"failpidchi2_$sig_region"+"sig",histoBuilders.eieo).fill(pcaldep, eidep)
				 if( pcaldep > 0 && eodep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsf_pcalvseo_passall"+"failpidchi2_$sig_region"+"sig",histoBuilders.calsf).fill(pcaldep/p, eodep/p)
				 if( pcaldep > 0 && eodep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsfmod_pcalvseo_passall"+"failpidchi2_$sig_region"+"sig",histoBuilders.eieo).fill(pcaldep/p, eodep)
				 if( pcaldep > 0 && eodep > 0 ) histos.computeIfAbsent(p_name+"_"+"mod_pcalvseo_passall"+"failpidchi2_$sig_region"+"sig",histoBuilders.eieo).fill(pcaldep, eodep)
				 */
			...
			/*
			if( p_range >= 0 ){
			    histos.computeIfAbsent(p_name+"_"+"calsf_pcalvsecal_prange"+"$p_range"+"_passall",histoBuilders.calsf).fill(pcaldep/p, (eidep + eodep)/p)			    
			    histos.computeIfAbsent(p_name+"_"+"calsfchi_prange"+"$p_range"+"_passall",histoBuilders.ecsf_chi).fill(pidchi2) 	//////////////////////////////////////////////////////////////
			    histos.computeIfAbsent(p_name+"_"+"calsfchi_prange"+"$p_range"+"_passall"+"_s$cal_sector",histoBuilders.ecsf_chi).fill(pidchi2) 	
			    
 			    //if( eodep > 0 && eidep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsf_eivseo_prange"+"$p_range"+"_passall",histoBuilders.calsf).fill(eidep/p, eodep/p)
			    //if( pcaldep > 0 && eidep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsf_pcalvsei_prange"+"$p_range"+"_passall",histoBuilders.calsf).fill(pcaldep/p, eidep/p)
			    //if( pcaldep > 0 && eidep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsf_pcalvseo_prange"+"$p_range"+"_passall",histoBuilders.calsf).fill(pcaldep/p, eodep/p)
			    //histos.computeIfAbsent(p_name+"_"+"calsf_eivseo_prange"+"$p_range"+"_passall",histoBuilders.calsf).fill(eidep/p, eodep/p)
			    //histos.computeIfAbsent(p_name+"_"+"calsf_pcalvsei_prange"+"$p_range"+"_passall",histoBuilders.calsf).fill(pcaldep/p, eidep/p)
			    //histos.computeIfAbsent(p_name+"_"+"calsf_pcalvseo_prange"+"$p_range"+"_passall",histoBuilders.calsf).fill(pcaldep/p, eodep/p)
			}
			 */

			if( p > 4.0 ){
			    histos.computeIfAbsent(p_name+"_"+"calsf_pcalvsecal_bigp_passall",histoBuilders.calsf).fill(pcaldep/p, (eidep + eodep)/p)			    
			    /*if( eodep > 0 && eidep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsf_eivseo_bigp_passall",histoBuilders.calsf).fill(eidep/p, eodep/p)
			    if( pcaldep > 0 && eidep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsf_pcalvsei_bigp_passall",histoBuilders.calsf).fill(pcaldep/p, eidep/p)
			    if( pcaldep > 0 && eidep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsf_pcalvseo_bigp_passall",histoBuilders.calsf).fill(pcaldep/p, eodep/p)
			     */
			...
			/*
			if( p_range >= 0 ){
			    histos.computeIfAbsent(p_name+"_"+"calsf_pcalvsecal_prange"+"$p_range"+"_fail_at_least1",histoBuilders.calsf).fill(pcaldep/p, (eidep + eodep)/p)			    
			    histos.computeIfAbsent(p_name+"_"+"calsfchi_prange"+"$p_range"+"_fail_at_least1",histoBuilders.ecsf_chi).fill(pidchi2) 	
			    histos.computeIfAbsent(p_name+"_"+"calsfchi_prange"+"$p_range"+"_fail_at_least1"+"_s$cal_sector",histoBuilders.ecsf_chi).fill(pidchi2) 	
			    
			    //if( eodep > 0 && eidep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsf_eivseo_prange"+"$p_range"+"_fail_at_least1",histoBuilders.calsf).fill(eidep/p, eodep/p)
			    // if( pcaldep > 0 && eidep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsf_pcalvsei_prange"+"$p_range"+"_fail_at_least1",histoBuilders.calsf).fill(pcaldep/p, eidep/p)
			    //if( pcaldep > 0 && eidep > 0 ) histos.computeIfAbsent(p_name+"_"+"calsf_pcalvseo_prange"+"$p_range"+"_fail_at_least1",histoBuilders.calsf).fill(pcaldep/p, eodep/p)
			    //histos.computeIfAbsent(p_name+"_"+"calsf_eivseo_prange"+"$p_range"+"_fail_at_least1",histoBuilders.calsf).fill(eidep/p, eodep/p)
			    //histos.computeIfAbsent(p_name+"_"+"calsf_pcalvsei_prange"+"$p_range"+"_fail_at_least1",histoBuilders.calsf).fill(pcaldep/p, eidep/p)
			    //histos.computeIfAbsent(p_name+"_"+"calsf_pcalvseo_prange"+"$p_range"+"_fail_at_least1",histoBuilders.calsf).fill(pcaldep/p, eodep/p)
			}
			 */
			if( p > 4.0 ){
